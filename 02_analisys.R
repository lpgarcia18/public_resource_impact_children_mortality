...
external_factors <- c("FERTILITY_RATE_LAGGED", "PLUS_65_YEARS_LAGGED", "LOG_GDP_PER_CAP_LAGGED", "LOG_OOP_PER_CAP_LAGGED",
			...
			"DELIVERY_ASSISTANCE_LAGGED", "DPT_LAGGED", "ELECTRICITY_LAGGED", "URBAN_RATE_LAGGED", "pop", "POP_DENS", "LONG", "LAT")
...
coef_var <- cbind(coef_var_public, coef_var_health, coef_var_other) %>% as.data.frame()
names(coef_var) <- c("Total", "Health", "Others")
coef_var$Models <- c("SuperLearner", "PS", "CBPS")

write.csv(coef_var, "bases/coef_var.csv", row.names = F)
...
model.fit_public_neo = FALSE, model.info = FALSE, digits = 3) 
...
model.fit_health_neo = FALSE, model.info = FALSE, digits = 3) 
...
model.fit_other_neo = FALSE, model.info = FALSE, digits = 3) 
...
model.fit_public_neo_u5 = FALSE, model.info = FALSE, digits = 3) 
...
model.fit_health_neo_u5 = FALSE, model.info = FALSE, digits = 3) 
...
model.fit_other_neo_u5 = FALSE, model.info = FALSE, digits = 3) 


#Extracting data from fit models
impact <- rbind(public_neo_impact$coeftable[2,c(1,2,3)] %>% data.frame() %>% t(),
...
impact$treatment <- c("Total public expenditure", 
			    "Health public expenditure",
			    "Public expenditure in other sectores")
impact$mortality_rate <- c(rep("NeoRt",3),rep("NeoU5Rt",3))
