...
p_cor_ext <- function(compare){ #function to calculate p value of pearson correlation and extract external factors with p <= 0.25
	biv_compare <-list()
...
p_cor_ext_reg <- function(compare, treatment){ #function to calculate p value of pearson correlation and extract external factors with p <= 0.25
	biv_compare <-list()
	for(i in 1:length(external_factors)){
		biv_compare[[i]] <- cor.test(x = as.matrix(dose_resp[,colnames(dose_resp) == compare]), 
					       y = as.matrix(dose_resp[,colnames(dose_resp) == external_factors[i]]))$p.value
...
impact_95$treatment <- c("Total public expenditure", 
					   "Health public expenditure",
					   "Public expenditure in other sectores")
impact_95$mortality_rate <- c(rep("NeoRt",3),rep("NeoU5Rt",3))

impact_99 <- rbind(public_neo_impact_99$coeftable[2,c(1,2,3)] %>% data.frame() %>% t(),
...
impact_99$treatment <- c("Total public expenditure", 
					   "Health public expenditure",
					   "Public expenditure in other sectores")
